...
// Enable AS7341
fn as7341_enable(i2c: &I2c, flag: bool) -> Result<()> {
...
// Enable LEDs
fn as7341_enable_leds(i2c: &I2c, flag: bool) -> Result<()> {
    // Switch to Bank 1
...
// Control LEDs
fn as7341_control_leds(i2c: &I2c, led: bool, mut current: u8) -> Result<()> {
...
fn as7341_atime_config(i2c: &I2c, value: u8) -> Result<()> {
...
fn as7341_astep_config(i2c: &I2c, value: u16) -> Result<()> {
...
// Config AGAIN
fn as7341_again_config(i2c: &I2c, mut value: u8) -> Result<()> {
    // Check value range
    if value > 10 {
        value = 10;
    }
...
// Start meaasure
fn as7341_start_measure(i2c: &I2c, mode: u8) -> Result<()> {
    // Check value range
    if mode != 0 && mode != 1 {
        return Ok(());
    }
...
// Get Channel Data
fn as7341_get_channel_data(i2c: &I2c, chan: u8) -> u16 {
...
fn as7341_read_spectral_data_one(i2c: &I2c) -> Result<()> {
    // Read channel data
...
fn as7341_read_spectral_data_two(i2c: &I2c) -> Result<()> {
    // Read channel data
...
fn main() -> Result<()> {
    ...
    // Initialize I2C communication with AS7341
    let mut i2c: I2c = I2c::new()?;

    // Set the I2C slave address to the device we're communicating with
    i2c.set_slave_address(AS7341_ADDR)?;    // Set AS7341 address

    /* 
    println!("Turn on AS3741 LEDs"); 

    // Enable LEDs
    as7341_enable_leds(&i2c, true)?;

    // Turn on LEDs
    for n in 0..20_u8 {
        println!("LED level = {}", n);
        as7341_control_leds(&i2c, true, n)?;
    ...
    // Turn off LEDs
    as7341_control_leds(&i2c, false, 0)?;

    // Disable LED
    as7341_enable_leds(&i2c, false)?;    
    */
    ...
    // AS7341 ATIME Config
    as7341_atime_config(&i2c, 100)?;

    // AS7341 ASTEP Config
    as7341_astep_config(&i2c, 999)?;

    // AS7341 AGAIN Config
    as7341_again_config(&i2c, 0x06)?;

    // Enable AS7341
    as7341_enable(&i2c, true)?;    

    // Read channel data
    for _ in 0..10 {
        as7341_start_measure(&i2c, 0)?;
        as7341_read_spectral_data_one(&i2c)?;
        as7341_start_measure(&i2c, 1)?;
        as7341_read_spectral_data_two(&i2c)?;
        println!("");
        thread::sleep(Duration::from_millis(1000));
    }
